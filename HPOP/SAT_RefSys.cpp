...
//
//------------------------------------------------------------------------------
Vector COE(Vector& r, Vector& v)
{
	Vector hbar(3), nbar(3),ebar(3),coe(7);
	double magr, magv, magh,magn;
	double c1,mu,rdotv;
	double a, ecc, incl, argp, omega, nu, m;
	double sme, hk, temp,cose,sine,e0;
	magr = Norm(r);
	magv = Norm(v);
	hbar = Cross(r, v);
	magh = Norm(hbar);
	...
	nbar(1) = hbar(0);
	nbar(2) = 0.0;
	//\BC\C6\CB\E3ƫ\D0\C4\C2\CAecc
	magn = Norm(nbar);
	mu = GM_Earth/1000000000;
	c1 = magv*magv - mu/magr;
	rdotv = Dot(r, v); //\B1\EA\C1\BF\BB\FD
	ebar(0) = (c1*r(0) - rdotv*v(0)) / mu;
	ebar(1) = (c1*r(1) - rdotv*v(1)) / mu;
	ebar(2) = (c1*r(2) - rdotv*v(2)) / mu;
	ecc = Norm(ebar);
	//\BC\C6\CB\E3\B0볤\D6\E1a
	sme = (magv*magv*0.5) - (mu / magr); //\BB\FAе\C4\DC
	a = -mu / (2.0 *sme);
	//\BC\C6\CB\E3\B9\EC\B5\C0\C7\E3\BD\C7incl
	hk = hbar(2) / magh;
	incl = acos(hk);
	//\BC\C6\CB\E3\C9\FD\BD\BB\B5\E3\B3ྭ
	temp = nbar(0) / magn;
	if (temp > 1.0)
		temp = 1.0;
	if (temp < -1.0)
	...
	if (nbar(1) < 0.0)
		omega = pi2 - omega;
	//\BC\C6\CB\E3\BD\FC\B5ص\E3\B7\F9\BD\C7
	argp = Ang(nbar, ebar);
	if (ebar(2) < 0.0)
		argp = pi2 - argp;
	//\BC\C6\CB\E3\D5\E6\BD\FC\B5\E3\BD\C7
		nu = Ang(ebar, r);
	if (rdotv < 0.0)
		nu = pi2 - nu;
	//\BC\C6\CB\E3ƽ\BD\FC\B5\E3\BDǣ\AC\CA\E4\C8\EBƫ\D0\C4\C2ʺ\CD\D5\E6\BD\FC\B5\E3\BD\C7
	...
	coe(0) = a; coe(1) = ecc; coe(2) = incl*Deg; coe(3) = omega*Deg; coe(4) = argp*Deg; coe(5) = nu*Deg; coe(6) = m*Deg;
	return coe; 
}

Matrix BodyOrbitMatrix(double R, double P, double Y)
{
	// \CA\E4\C8\EB\B5\C4double\C0\E0\D0\CDR,P,Y\B5\A5λΪrad,\C8\F4\CE\C0\D0ǲ\BB\B2\E0\B0ڣ\AC\BE\D8\D5\F3Ϊ\B5\A5λ\D5\F3
	// Transformation from satellite body to orbit\A3\AC\B9\F6\B6\AF\BD\C7R\CA\D3Ϊ\D7\F3\D3\D2\CAӳ\A1\A3\A8\D7\F3\CAӽǶ\C8Ϊ\B8\BA\A3\A9\A3\AC\B8\A9\D1\F6\BD\C7P\CA\D3Ϊǰ\BA\F3\CAӳ\A1\A3\A8ǰ\CA\D3Ϊ\B8\BA\A3\A9
	return  R_z(Y)*R_x(R)*R_y(P);
}

//Matrix ECIOrbitMatrix(double ui, double in, double om)
//{
//	Matrix U(3, 3);
//	U.M[0][0] = -cos(om)*sin(ui) - sin(om)*cos(in)*cos(ui);  U.M[0][1] = cos(om)*cos(in)*cos(ui) - sin(om)*sin(ui);  U.M[0][2] = cos(ui)*sin(in);
//	U.M[1][0] = -sin(om)*sin(in);  U.M[1][1] = cos(om)*sin(in);  U.M[1][2] = -cos(in);
//	U.M[2][0] = sin(om)*cos(in)*sin(ui) - cos(om)*cos(ui);  U.M[2][1] = -sin(om)*cos(ui) - cos(om)*cos(in)*sin(ui);  U.M[2][2] = -sin(in)*sin(ui);
//	return U;
//	// \CA\E4\C8\EB\B5\C4double\C0\E0\D0\CDui,i,om\B5\A5λΪrad
//	// Transformation from eci to orbit\A3\AC\C8\C6Z\D6\E1\C4\E6ʱ\D5\EBתascend\A3\AC\C8\C6X\D6\E1\C4\E6ʱ\D5\EB\D0\FDתincli,\D4\D9\C8\C6Z\D6\E1\C4\E6ʱ\D5\EB\D0\FDתperigee
//	//return  R_y(-(pi/2.0+ui))*R_x(-(pi/2.0-in))*R_z(om);
//}
