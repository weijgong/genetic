...
Vector::Vector(double x, double y, double z,   // 7dim-Vector
	           double X, double Y, double Z,
			   double my)
	: n(7)
{
...
// Dot product, Ang, norm, cross product
...
double Ang(const Vector& left, const Vector& right)
{
	double magv1, magv2, theta,temp;
	magv1 = Norm(left);
	magv2 = Norm(right);
	temp = Dot(left, right) / (magv1*magv2);
	if (temp > 1.0)
		temp = 1.0;
	if (temp < -1.0)
		temp = -1.0;
...
Matrix ECIOrbitMatrix(double ui, double in, double om)
{
	...
	return U;
	// 输入的double类型ui,i,om单位为rad
	// Transformation from eci to orbit，绕Z轴逆时针转ascend，绕X轴逆时针旋转incli,再绕Z轴逆时针旋转perigee
